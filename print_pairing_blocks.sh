#!/usr/bin/env bash
# print_pairing_blocks.sh — reprint, for every peer Levante already paired via
...
# the self-test for real against Levante's own live listener each time.
#
...
emit_one() {
  ...
  code="$(curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST "http://${SELF_ADDR}/inbox" \
    ...
    -d '{"subject":"selftest","body":"levante receiving-half self-test; safe to delete"}' || echo "000")"
