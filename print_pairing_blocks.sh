#!/usr/bin/env bash
# print_pairing_blocks.sh — reprint, for every peer Chinook already paired via
...
# the self-test for real against Chinook's own live listener each time.
#
...
emit_one() {
  ...
  code="$(curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST "http://${SELF_ADDR}/inbox" \
    ...
    -d '{"subject":"selftest","body":"chinook receiving-half self-test; safe to delete"}' || echo "000")"
