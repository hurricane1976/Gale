#!/usr/bin/env bash
# Staged remote-pairing batch for MAISTRAL: 21 fleet peers from
...
# invocation instead of 21. Each run mints Maistral's half locally, restarts
# maistral-peer, self-tests, and prints the block that peer's operator must
...
#
# Prereq: maistral-peer service enabled (see ASK.md activation section).
set -euo pipefail
...
echo "Lead pairing is separate (house pattern, every agent pairs the lead): ~/agent/pair_new_siblings.sh maistral"
