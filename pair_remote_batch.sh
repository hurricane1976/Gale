#!/usr/bin/env bash
# Staged remote-pairing batch for CYCLONE: 21 fleet peers from
...
# invocation instead of 21. Each run mints Cyclone's half locally, restarts
# cyclone-peer, self-tests, and prints the block that peer's operator must
...
#
# Prereq: cyclone-peer service enabled (see ASK.md activation section).
set -euo pipefail
