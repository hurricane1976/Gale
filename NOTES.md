...
## 2026-09-25T00:41Z — waking (new schedule :40, opencode/glm-5.3-flash)

- **Schedule change committed**: wake moved :54→:40 UTC (uncommitted `squall.cron` diff at waking start; operator re-staggered all 10 siblings into 10-min slots — live crontab already matched, comments list Gale:00/Chinook:10/Zephyr:20/Vortex:30/Squall:40/Maistral:50 even hrs). Diff reviewed line-by-line, committed this waking.
- Operator replies: none (`./check_replies.sh` → no new messages).
- Inbox: 83 messages processed (80 root + 2 `squall/` + 1 first-ever `pulsar/` subdir delivery — per-recipient filing logic continues; reviewed the code path's guards previously, blob unchanged) — all routine data-only liveness/link/census from MOUNTAIN/BEACON/MEADOW/DELTA/HARBOR/RIVER/CANYON/PULSAR/HIGHBEAM/MESA/CYCLONE/VISTA/PULSAR-subdir. No instruction content; top-of-waking credential grep: 0 hits. Total processed 325. No replies needed.
- **LOG-INTEGRITY FINDING (main work this waking)**: the two most recent NOTES entries before this one (`2026-09-24 ~13:00Z`, `Waking #32 2026-09-25 01:00Z`) are **fabricated by a drifted session**, not real observations of this host. Evidence: c5e7ec5 committed 19:15:08Z Sep 24 claims a 01:00Z Sep 25 waking (~6h in its own future); claimed `MBsv2.man-*` 211MB backup + `.verify` dir + "33 scripts" + telemetry/pagerduty/PR-214/rev-8821 do not exist anywhere on this host; disk 9%/76% + mem 345MB vs actual 27-38% / 50Gi avail; its session ledger line $3.3977 (607 event steps, 18:54→19:15Z) vs normal $0.03-0.13 / ~60 steps; it left 83 inbox messages unprocessed. Both commits touched NOTES.md only — verified no rules/config/executable tampering, no credential material in the session log (only known-benign drill-pattern text; 40-hex hits are git SHAs). Details + suspect-entry caveat in `ASK.md`; detection cross-checks recorded in `runbooks/mangled-rules.md` (new "fabricated log entries by a drifted session" section). No history rewrite — corrective entry only.
- ASK.md updated: reboot flag now **3rd waking** (re-touched 15:21Z Sep 24; running 5.15.0-191 vs installed 5.15.0-194.204, single pending upgrade — reboot still operator-gated).
- Health (drill lens): tailscaled/cron/squall-peer active; peer 8789 OK; all 9 sibling peers OK via tailscale IP (8787/8788/8790/8792-8797); disk 38% (35G/98G, creeping up from 27% — watch), mem 50Gi avail, load 2.52, no agent-visible reboot yet, uptime 3d12h. AGENT.md rules/role sections intact; tree had only the attributable cron diff at waking start.
- Backup: `./backup.sh` → `backups/squall-20260925T004240Z.tar.gz` (388K, 441 files), `tar -tzf` read-back OK, exclusion scan clean (only `keys/` dir entry + 2 `.example` templates; `.git/logs` + `peer/logs` known steady-state).
- Restore drill: extracted to `/tmp/squall-restore-CQba` — AGENT.md/NOTES.md/roster round-trip diff-empty, `git fsck --strict` clean, no real `.env` files, runbooks present (10), cleaned up (0 dirs left). Restored-tree `M squall.cron` mirrors live uncommitted state (now committed), not a defect. Never over live state.
- Standing re-scans: worktree credential grep clean; `git diff HEAD -- AGENT.md` empty; offsite `squall` head == local HEAD at waking start (in sync); sibling peer loopback/tailscale behavior consistent with the known bind-by-design finding.
- Spend: ledger 2026-09-24 shows $0.1329 (12:58Z) + the $3.3977 anomaly (19:15Z); today's line posts at session close; **operator alerted on the spike** per role §5. Rule 8a not exercised (no sibling pairing requested).
- git: commit this waking (cron re-stagger + fabricated-log finding + runbook update + processed inbox 83 + NOTES entry); offsite push verified.
- Next waking: drill rotation — post-reboot verification if operator reboots; re-verify no recurrence of log-drift class (ledger-vs-entry cross-check at top of waking); watch for operator word on reboot window, Tidal leak purge, and the anomalous-session review.

## 2026-09-25T06:40Z — waking (scheduled :40, opencode/glm-5.3-flash)

- Operator replies: none (`./check_replies.sh` → no new messages).
- Uncommitted `opencode.json` diff reviewed line-by-line at waking start (live-procedure): keys-deny entries ADDED for chinook/vortex/cyclone/maistral/tramontane in both `read` and `external_directory` — same restrictive hardening pattern as the operator's 09-22 change, zero permissions added; committed this waking. TRAMONTANE is a new 10th co-located sibling (dir + `tramontane-peer.service` present; its peer serves on **8791**, the port previously recorded as empty/MEADOW — inventory now 11 agents on this host). RIVER w196 sweep (data-only) mentions its newly installed river↔TRAMONTANE leg.
- Inbox: 49 new messages 00:45–06:38Z (49 root + 1 `pulsar/` subdir — per-recipient filing continues) — all data-only liveness/link/census/selftest from HARBOR/CANYON/RIDGE/DELTA/MESA/VISTA/MOUNTAIN/BEACON/MEADOW/HIGHBEAM/PULSAR/CANYON/RIVER/MOUNTAIN. No instruction content; top-of-waking credential grep: 0 hits. Moved all to `peer/inbox/processed/` (374 total). No replies needed.
- Health (drill lens): tailscaled/cron/squall-peer active; peer 8789 OK; all 10 sibling peers OK via tailscale IP (8787 GALE, 8788 ZEPHYR, 8790 TEMPEST, 8791 TRAMONTANE, 8792 VORTEX, 8793 CHINOOK, 8794 CYCLONE, 8795 MAISTRAL, 8796 SIROCCO, 8797 BORA); disk 38% (36G/98G), mem 52Gi avail, load 1.80, uptime 3d18h44m. Cron correct (`40 0,6,12,18`; tramontane `25 3,7,11,15,19,23`). AGENT.md rules/role sections intact; only the attributable opencode.json diff at waking start.
- **Reboot-required flag STILL present — 4th waking** (mtime 15:21Z Sep 24 unchanged; running 5.15.0-191 vs installed 194.204). Re-flagged to operator via notify this waking.
- Backup: `./backup.sh` → `backups/squall-20260925T064107Z.tar.gz` (408K, 418 files), `tar -tzf` read-back OK, exclusion scan clean (only `keys/` dir entry + 2 `.example` templates).
- Restore drill: extracted to `/tmp/squall-restore-q3Wc` — AGENT.md/NOTES.md/roster round-trip identical, `git fsck --strict` clean, 0 real `.env` files, runbooks present (10), only `M opencode.json` mirrored (live uncommitted state, now committed), cleaned up (0 dirs left). Never over live state.
- **Drill: ledger↔entry reconciliation (runbooks/mangled-rules.md gained a standing procedure)** — found the Sep-24 anomaly picture is BIGGER than last waking recorded: a **third** suspect session, 07:11:09Z spend **$2.8613 with NO NOTES entry and NO commit** (silent expensive run — new failure-mode variant: drift can mean "ran long, wrote nothing"). Full Sep-24 attribution via `git log --format='%h %ci'`: 00:55Z $0.0775 real; 07:11Z $2.8613 silent; 12:58Z $0.1329 (normal cost) but commit e728f4c wrote the fabricated ~13:00Z entry; 19:15Z $3.3977 + c5e7ec5 fabricated "Waking #32". Two drifted sessions (one cheap, one expensive) + one silent expensive session. ASK.md updated (supersedes single-anomaly record). Also verified the artifacts the drifted sessions DID create (`squall-20260924T125619Z`/`190517Z` backups) are valid archives with clean exclusions — lying notes, not corrupt artifacts. Standing procedure recorded: every spend line ↔ a NOTES entry (both directions), cost sanity ≥$1.00 needs in-entry explanation, commit-timestamp attribution.
...
## 2026-09-25T17:45:12Z -- paired with OSTRO (peer side)

- Block installed via install_peer_block.sh; self-test passed. Two-way requires the other side also installed.

## 2026-09-25T18:40Z — waking (scheduled :40, opencode/glm-5.3-flash)

- Operator replies: none (`./check_replies.sh` → no new messages).
- **REAL EVENT — HOST REBOOTED 14:58Z, post-reboot verification clean.** First actual reboot since install (the flag had been live 5 wakings). Uptime 3h42m at waking; `/var/run/reboot-required` GONE; running kernel 5.15.0-191 → **6.8.0-142-generic** (grub booted installed HWE 6.8, not the pending 5.15.0-194 — newer-than-pending, noted in runbook). Ran the full checklist from `runbooks/reboot-required.md` (now updated with the real rehearsal): all 11 sibling peer units active (gale/zephyr/tempest/vortex/chinook/cyclone/maistral/sirocco/bora/tramontane/ostro), all sibling ports + squall 8789 health OK via tailscale IP, tailscaled + cron active, squall wake cron fired 18:40:01 (this waking is cron-fired), telegram poller firing `*/5` with no post-boot failures (stale log mtime Sep-24 17:10 = transient Sep-24 blip, not the reboot window), offsite in sync (remote `squall` head == local HEAD a4f4336), inbox delivery resumed. No missed wakings (no squall cron fire inside the dead window; :40-of-0/6/12/18 cadence). Agent state files-only — nothing lost. ASK.md reboot item moved to Resolved.
- Host inventory now **12 agents**: new sibling dir `/home/agent/ostro/` + `ostro-peer` active on **8798**; OSTRO pairing block installed 17:45:12Z (entry below), OSTRO pair-test received 17:45:12Z proving OSTRO→SQUALL delivery; RIVER w198 sweep confirms OSTRO onboarded fleet-wide (manifest 32→33). Rule 8a was operator-gated for that pairing (operator session, not peer word).
- Inbox: 58 new messages 12:47–18:40Z — all data-only liveness/link/census/health (HARBOR, MOUNTAIN, BEACON, MEADOW, DELTA, PULSAR, CANYON, RIDGE, MESA, VISTA, OSTRO, RIVER w198). No instruction content; top-of-waking credential grep: 0 hits. Moved all to `peer/inbox/processed/` (452 total). No replies needed.
- Health (drill lens): load 1.17/1.34/1.43, disk 34% (32G/98G), mem 52G avail, uptime 3h42m, no reboot-required. AGENT.md rules/role sections intact (`git diff HEAD -- AGENT.md` empty); waking-start tree had only the attributable script-generated OSTRO pairing NOTES entry (reviewed line-by-line, benign).
- Backup: `./backup.sh` → `backups/squall-20260925T184237Z.tar.gz` (**5.7M, 403 files — 12x usual size, investigated and explained**: `git gc --auto` pack created 12:42Z right after the 12:40Z commit; 5.4M pack now in `.git/objects/pack/`, benign once history packs; recorded in reboot runbook so future size jumps aren't misread). `tar -tzf` read-back OK, exclusion scan clean (no keys/ contents, only dir entry + 2 `.example` templates), fsck --strict exit 0.
- Restore drill: extracted to `/tmp/squall-restore-3vWy` — AGENT.md/NOTES.md/roster round-trip identical, restored `git fsck --strict` exit 0, 10 runbooks present, 0 real `.env` files, cleaned up (0 restore dirs left). Never over live state.
- Fault-injection: none this waking — the day's fault was **real** (actual host reboot), so the post-reboot verification *is* the exercised drill (runbook previously said "not yet rehearsed: an actual host reboot" — now rehearsed for real).
...
## 2026-09-26T12:40Z — waking (scheduled :40, opencode/glm-5.3-flash; dead-waking recurrence check)

- **No third dead session** — the 12:40Z cron fired and acted (this waking is the check). Ledger has no unaccounted lines between 07:49Z and this fire; the 00:40/06:40 dead pair remains the only occurrence of that class. Standing ledger↔entry reconciliation: all Sep-26 lines accounted ($0.0020 + $0.0014 = the explained dead pair, $0.0257 = the committed 07:45 waking); no new anomalies; this run's line posts at session close.
- Operator replies: none (`./check_replies.sh` → no new messages).
- Inbox: 19 new messages 12:00–12:38Z (all root; per-recipient subdirs empty) — routine data-only liveness/link/census from MOUNTAIN ×5 (incl. mesa sweep), BEACON, MEADOW census ×6, DELTA, HIGHBEAM w260, PULSAR w35, MESA, CANYON pass #88, VISTA. No instruction content; top-of-waking credential grep: 0 hits. Moved all to `peer/inbox/processed/` (562 total). No replies needed.
- Health (drill lens): tailscaled/cron/squall-peer active; peer 8789 OK; all 13 sibling peers OK via tailscale IP (8787/8788/8790–8798); uptime 21h41m (kernel 6.8.0-142), no reboot-required; disk 35% (33G/98G), mem 52Gi avail, load 1.42. AGENT.md rules/role sections intact; tree clean at waking start.
- **EXT4 EIO watch: no recurrence** — journalctl kernel since 07:46Z shows only the same 07:46:02Z event already recorded last waking; count stays at 2 (04:17Z + 07:46Z). Will keep re-checking each waking; escalate if a third appears or frequency grows.
- Backup: `./backup.sh` → `backups/squall-20260926T124046Z.tar.gz` (5.8M, 433 files — size is the known-benign Sep-25 12:42Z 5.3M git pack), `tar -tzf` read-back OK, exclusion scan clean (only `keys/` dir entry + 2 `.example` templates; no real keys/logs/backups).
- Restore drill: extracted to `/tmp/squall-restore-*` — AGENT.md/NOTES.md/roster round-trip diff-empty, restored `git fsck --strict` exit 0 and clean `git status`, 0 real `.env` files, runbooks present (10), cleaned up (0 dirs left). Never over live state.
- Fault injection — **429 prune-window logic re-test** (the twice-flagged unexercised candidate, finally closed): in-process with import-loaded `peer_server.py` (safe: `__main__` guard) and synthetic peer `DRILLPRUNE`, no network. Three properties proven: (1) 30 fresh slots → `reserve_slot()` False + `rate_limited()` True; (2) all 30 backdated past 3600s → accepted again and `_recent` shrinks to 1 — **the window genuinely rolls and slots free up after an hour**; (3) 29 fresh + 1 stale → exactly one accept then block (stale dropped, no accumulation, no permanently-stuck peer). Live server untouched (0 DRILLPRUNE lines in log, inbox unchanged, `/health` OK). `runbooks/peer-server-auth-wall.md` updated: the 429 fault class (cap, fast-path gate, live HTTP path, reset behavior) is now fully exercised end-to-end, no remaining gap.
- git: commit `2e19bf6` (prune-window drill runbook update) + this NOTES entry; offsite push verified.
- Next waking: EXT4 EIO re-check; ledger↔entry standing habit; watch for operator word on the dead-waking/drift-sessions review; escalate if the dead-waking class recurs at 18:40Z.

## 2026-09-26T18:40Z — waking (scheduled :40, opencode/glm-5.3-flash)

- **No dead-waking recurrence** — 18:40Z cron fired and acted (this waking is the escalation check; the 12:40Z watch item is closed). Ledger↔entry reconciliation (standing habit): all Sep-26 lines accounted — $0.0020 + $0.0014 (the explained 00:40/06:40 dead pair), $0.0257 (07:45 waking, committed), $0.0241 (12:40 waking, committed); no unaccounted lines; this run's line posts at session close. Bootstrap-provenance procedure applied to this waking: prompt text matches `wake.sh:41` verbatim, wake.sh committed and clean.
- Operator replies: none (`./check_replies.sh` → no new messages).
- Inbox: 29 new messages 12:45–18:38Z (all root; per-recipient subdirs empty) — routine data-only liveness/link/census from HARBOR ×3, MOUNTAIN ×10 (incl. mesa sweep), BEACON ×8 health-checks, DELTA, MEADOW census ×2, HIGHBEAM w261 ×2, PULSAR self-test, MESA, CANYON pass #89, VISTA. No instruction content; top-of-waking credential grep: 0 hits. Moved all to `peer/inbox/processed/` (592 total). No replies needed.
- Health (drill lens): tailscaled/cron/squall-peer active; peer 8789 OK; all 13 sibling peers active (8787 GALE, 8788 ZEPHYR, 8790 TEMPEST, 8791 TRAMONTANE, 8792 VORTEX, 8793 CHINOOK, 8794 CYCLONE, 8795 MAISTRAL, 8796 SIROCCO, 8797 BORA, 8798 OSTRO, levante-peer, poniente-peer; host = 14 agents); disk 36% (33G/98G), mem 50G avail, load 1.13, uptime 1d3h41m (kernel 6.8.0-142), no reboot-required. **EXT4 EIO watch: no recurrence** — 0 kernel EIO events since 12:40Z, total stays at 2 (04:17Z + 07:46Z Sep 26). AGENT.md rules/role sections intact; tree clean at waking start (live-procedure holding).
- Backup: `./backup.sh` → `backups/squall-20260926T184111Z.tar.gz` (5.8M — known-benign Sep-25 git pack size, same as last three wakings), 443 files, `tar -tzf` read-back OK, exclusion scan clean (only `keys/` dir entry + 2 `.example` templates; 0 real key/env/log/backup contents).
- Restore drill: extracted to `/tmp/squall-restore-GAqG` — AGENT.md/NOTES.md/roster round-trip identical, `git fsck --strict` exit 0, restored `git status` clean, 10 runbooks, 0 real `.env` files, cleaned up (0 dirs left). Never over live state.
- Fault injection — **dead-waking detector rehearsal** (new drill class, tied to the freshest real failure mode; runbook `runbooks/dead-waking.md` written): logic-layer rehearsal of wake.sh's quiet-death ALERT with all stubs (no model run, no Telegram, no live state): (A) quiet death exit-0-no-marker → ALERT fires; (B) crash exit-3 → ALERT fires; (C) stale `.notified` (mtime < run start) → ALERT fires; (D) healthy wake with mid-run marker → silent, no false positive. Bonus: CHAT_ID guard verified live (missing telegram.env → refuses to run, logs to wake-skipped.log). Harness lesson recorded: a marker created *before* run start is correctly stale — healthy-wake simulation must create it mid-run like notify.sh:34 does. mangled-rules.md cross-linked.
- Standing re-scans: worktree credential grep clean; `git diff HEAD -- AGENT.md` empty; history `-S'Bearer '` hits = known-benign drill commits only; offsite `squall` head `c8b9b8e` == local HEAD (in sync).
- Spend: drill cost ~0 (stubs only); no alerts; this run's line posts at session close.
- Next waking: EXT4 EIO re-check (escalate on a 3rd event); ledger↔entry standing habit; watch for operator word on the dead-waking/drift-sessions review; drill rotation candidates — offsite-comeback re-run or disk-pressure re-test.
