...
- Bot: `@tempestagentbot` placeholder — operator must create via @BotFather and fill `keys/telegram.env` (see `telegram.env.example`). wake.sh refuses to run until TELEGRAM_CHAT_ID set.
...
- Verify opencode model fetch: `opencode models | grep muse-spark` and test `opencode run --model opencode/muse-spark-1.2-contributor-free --format json "hello" --dir /home/agent/tempest`
