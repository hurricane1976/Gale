...
- **Telegram bot not yet created for ZEPHYR** — placeholder `@zephyragentbot` in AGENT.md. Create via @BotFather, fill `keys/telegram.env` with TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID (same chat id as Gale: 8986669804, or new). wake.sh will refuse to run until set (same guard as Gale `wake.sh:25`).
