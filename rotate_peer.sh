...
#
#   ./rotate_peer.sh stage  <PEER_NAME>   mint a new token; Cyclone then accepts BOTH tokens
...
#
# Sequence:
#   1. stage on Cyclone          -> prints the new block for the peer
#   2. peer ADDS the new token as an additional accepted token for CYCLONE (keeps the old),
#      restarts its listener, and sends Cyclone a message with the new token
#   3. finish on Cyclone         -> old token rejected, Cyclone now sends with the new one
#   4. peer removes the old token from its side
# On any failure this script restores the backup it made and restarts cyclone-peer.
set -euo pipefail
umask 077
...
rollback() {
    echo "FAILED -- restoring $PEERS from $BACKUP and restarting cyclone-peer" >&2
    cp -p "$BACKUP" "$PEERS"
    sudo systemctl restart cyclone-peer || true
}
trap rollback ERR

# POST a test message with the given token to Cyclone's own listener; echo "<http> <from>".
...
restart() {
    sudo systemctl restart cyclone-peer
    sleep 2
    systemctl is-active --quiet cyclone-peer
}

if [[ "$CMD" == "stage" ]]; then
    NEW="$(openssl rand -hex 32)"
    [[ "${#NEW}" -eq 64 ]] || { echo "token generation failed" >&2; exit 1; }
    CYCLONE_NEW_TOKEN="$NEW" python3 "$HELPER" stage "$PEERS" "$NAME"
    ...
    printf '\n## %s -- rotation STAGED for %s\n\n- Operator ran rotate_peer.sh stage. Cyclone now accepts old + new token and still sends with the old one. Waiting for %s to install the new token; then `rotate_peer.sh finish %s`. Token not recorded here.\n' \
        "$(date -u +%FT%TZ)" "$NAME" "$NAME" "$NAME" >> NOTES.md
    cat <<EOF

=================== ROTATION STAGED FOR $NAME ===================
Nothing is broken: Cyclone accepts BOTH tokens and still sends with the OLD one.

Give $NAME's side this block. Tell them to ADD it as an additional accepted
token for CYCLONE (keep the old one for now), restart their listener, then send
Cyclone a message. If their config can hold only ONE token per name, tell them to
swap it right after you run 'finish' -- expect a brief 401 on Cyclone->$NAME until then.
...
Rotation for $NAME is complete: the old token is rejected and Cyclone now sends with the new one.
