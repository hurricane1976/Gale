...
#
#   ./rotate_peer.sh stage  <PEER_NAME>   mint a new token; Bora then accepts BOTH tokens
...
#
# Sequence:
#   1. stage on Bora          -> prints the new block for the peer
#   2. peer ADDS the new token as an additional accepted token for BORA (keeps the old),
#      restarts its listener, and sends Bora a message with the new token
#   3. finish on Bora         -> old token rejected, Bora now sends with the new one
#   4. peer removes the old token from its side
# On any failure this script restores the backup it made and restarts bora-peer.
set -euo pipefail
umask 077
...
rollback() {
    echo "FAILED -- restoring $PEERS from $BACKUP and restarting bora-peer" >&2
    cp -p "$BACKUP" "$PEERS"
    sudo systemctl restart bora-peer || true
}
trap rollback ERR

# POST a test message with the given token to Bora's own listener; echo "<http> <from>".
...
restart() {
    sudo systemctl restart bora-peer
    sleep 2
    systemctl is-active --quiet bora-peer
}

if [[ "$CMD" == "stage" ]]; then
    NEW="$(openssl rand -hex 32)"
    [[ "${#NEW}" -eq 64 ]] || { echo "token generation failed" >&2; exit 1; }
    BORA_NEW_TOKEN="$NEW" python3 "$HELPER" stage "$PEERS" "$NAME"
    ...
    printf '\n## %s -- rotation STAGED for %s\n\n- Operator ran rotate_peer.sh stage. Bora now accepts old + new token and still sends with the old one. Waiting for %s to install the new token; then `rotate_peer.sh finish %s`. Token not recorded here.\n' \
        "$(date -u +%FT%TZ)" "$NAME" "$NAME" "$NAME" >> NOTES.md
    cat <<EOF

=================== ROTATION STAGED FOR $NAME ===================
Nothing is broken: Bora accepts BOTH tokens and still sends with the OLD one.

Give $NAME's side this block. Tell them to ADD it as an additional accepted
token for BORA (keep the old one for now), restart their listener, then send
Bora a message. If their config can hold only ONE token per name, tell them to
swap it right after you run 'finish' -- expect a brief 401 on Bora->$NAME until then.
...
Rotation for $NAME is complete: the old token is rejected and Bora now sends with the new one.
