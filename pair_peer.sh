#!/usr/bin/env bash
# Pair Gale with ONE fleet peer: mint a shared token, install PONIENTE's half in
# keys/peers.env, restart poniente-peer, self-test the receiving side, and print
# the block the PEER's side must install.
#
# Run by the OPERATOR, by hand (AGENT.md rule 8: Gale never mints or installs
# peer tokens on its own). The token is printed only to your terminal, once.
#
...
sudo systemctl restart poniente-peer
sleep 2
systemctl is-active --quiet poniente-peer || { echo "FAIL: poniente-peer not active after restart" >&2; exit 1; }

# --- self-test of Gale's receiving half (token never echoed) ---------------
CODE="$(curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST "http://${SELF_ADDR}/inbox" \
    -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" \
    -d '{"subject":"selftest","body":"poniente receiving-half self-test; safe to delete"}')"
...
After $NAME confirms, test two-way from Gale with:
  ./send_to_peer.sh $NAME "hello from PONIENTE" "pair test"
