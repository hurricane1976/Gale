#!/usr/bin/env bash
# Pair Tramontane with ONE fleet peer: mint a shared token, install Tramontane's half
# in keys/peers.env, restart tramontane-peer, self-test the receiving side, and
# print the block the PEER's side must install.
#
# Run by the OPERATOR, by hand (AGENT.md rule 8: Tramontane never mints or
...
sleep 2
systemctl is-active --quiet tramontane-peer || { echo "FAIL: tramontane-peer not active after restart" >&2; exit 1; }

# --- self-test of Tramontane's receiving half (token never echoed) -------------
CODE="$(curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST "http://${SELF_ADDR}/inbox" \
    -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" \
    -d '{"subject":"selftest","body":"tramontane receiving-half self-test; safe to delete"}')"
...
After $NAME confirms, test two-way from Tramontane with:
