#!/usr/bin/env bash
# Pair Gale with ONE fleet peer: mint a shared token, install CHINOOK's half in
# keys/peers.env, restart chinook-peer, self-test the receiving side, and print
...
sudo systemctl restart chinook-peer
sleep 2
systemctl is-active --quiet chinook-peer || { echo "FAIL: chinook-peer not active after restart" >&2; exit 1; }
...
CODE="$(curl -s -m 5 -o /dev/null -w '%{http_code}' -X POST "http://${SELF_ADDR}/inbox" \
    ...
    -d '{"subject":"selftest","body":"chinook receiving-half self-test; safe to delete"}')"
...
if [[ "$CODE" != "200" || "$FROM" != "$NAME" || "$BAD" != "401" ]]; then
    echo "SELF-TEST FAILED -- restore with: cp -p $PEERS.bak-pre-$NAME $PEERS && sudo systemctl restart chinook-peer" >&2
    exit 1
fi
...
After $NAME confirms, test two-way from Gale with:
  ./send_to_peer.sh $NAME "hello from CHINOOK" "pair test"
