#!/usr/bin/env python3
"""Dynamic Telegram command handler for Bora.
...
trimmed to Bora's needs. For each new message *from the operator's exact
chat id*:
  ...
  * anything else -> appended to .telegram_incoming so the next waking's
    ...
    stays visible until Bora resolves it.
...
def send(text):
    ...
    if len(text) > MAX_MSG:
        text = text[:MAX_MSG] + "\n… (truncated)"
    data = urllib.parse.urlencode({"chat_id": CHAT_ID, "text": "[BORA] " + text}).encode()
    try:
