...
UNITS = ["tailscaled", "gale-peer", "zephyr-peer", "squall-peer", "tempest-peer", "vortex-peer", "cyclone-peer", "maistral-peer", "sirocco-peer", "bora-peer", "chinook-peer", "cron"]
...
def send(text):
    ...
    if len(text) > MAX_MSG:
        text = text[:MAX_MSG] + "\n… (truncated)"
    data = urllib.parse.urlencode({"chat_id": CHAT_ID, "text": "[CHINOOK] " + text}).encode()
    try:
