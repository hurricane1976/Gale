#!/usr/bin/env python3
"""Dynamic Telegram command handler for Cyclone.
...
trimmed to Cyclone's needs. For each new message *from the operator's exact
chat id*:
  ...
  * anything else -> appended to .telegram_incoming so the next waking's
    ...
    stays visible until Cyclone resolves it.
...
UNITS = ["tailscaled", "gale-peer", "zephyr-peer", "squall-peer", "tempest-peer", "vortex-peer", "cyclone-peer", "cron"]
...
def send(text):
    ...
    if len(text) > MAX_MSG:
        text = text[:MAX_MSG] + "\n… (truncated)"
    data = urllib.parse.urlencode({"chat_id": CHAT_ID, "text": "[CYCLONE] " + text}).encode()
    try:
...
def cmd_help(_arg):
    return (
        "Cyclone commands (operator only):\n"
