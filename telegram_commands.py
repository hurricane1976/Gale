#!/usr/bin/env python3
"""Dynamic Telegram command handler for Tramontane.
...
trimmed to Tramontane's needs. For each new message *from the operator's exact
chat id*:
  ...
  * anything else -> appended to .telegram_incoming so the next waking's
    ...
    stays visible until Tramontane resolves it.
...
UNITS = ["tailscaled", "gale-peer", "zephyr-peer", "squall-peer", "tempest-peer", "vortex-peer", "cyclone-peer", "maistral-peer", "sirocco-peer", "tramontane-peer", "cron"]
...
def send(text):
    ...
    if len(text) > MAX_MSG:
        text = text[:MAX_MSG] + "\n… (truncated)"
    data = urllib.parse.urlencode({"chat_id": CHAT_ID, "text": "[TRAMONTANE] " + text}).encode()
    try:
...
def cmd_help(_arg):
    return (
        "Tramontane commands (operator only):\n"
