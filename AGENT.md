...
`~/agent/pair_new_siblings.sh vortex`, and the 21 remote peers (see
...
Cyclone) would each need their own per-pair operator go-ahead under rule 8a;
the tooling exists (`~/agent/pair_siblings.sh vortex <sibling>`) but is not
staged.
